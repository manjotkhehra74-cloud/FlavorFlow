...
class _HrMateAppState extends State<HrMateApp> {
  ...
  Widget build(BuildContext context) {
    ...
    // (ARCHITECTURE.md §4 — that produced the 1-px text build).
    Widget clamped(BuildContext ctx, Widget child) => MediaQuery(
          data: MediaQuery.of(ctx).copyWith(
            textScaler: MediaQuery.of(ctx).textScaler.clamp(minScaleFactor: 0.9, maxScaleFactor: 1.3),
          ),
          child: child,
        );
    if (!auth.ready) {
...
/// Opaque navy, icon + spinner, NO text (ARCHITECTURE.md §4). Shown only
