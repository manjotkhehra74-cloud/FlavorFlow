...
| `0011-codetest-nonblocking.patch` | `42317f6` | codemagic.yaml: `flutter test` non-blocking (dono workflows) — blank-body investigation te test fail hoyi toh report log vich rehda a par build continue hove te diagnostic APK (laal DIAG strip wala 3.2.0+33) bana a. 1 file |
| `0010-phase7-diag-api-fix.patch` | `42317f6` | 0009 da analyze error: `debugGetDiagString` Element te nahi a — `debugDescribeChildren().toStringDeep()` use ki. 1 file (test only) |
| `0009-phase7-blank-diag-report.patch` | `93b5220` | test ab crash-free run hunda par home content na dikhda (header a, page na) — exact reason pata karne layi test vich full diagnostic report: shell CHILD ka runtimeType, widget counts (HomePage/Scaffold/ListView/etc), rects, text probes, aur HomePage mile toh poora AppShell tree dump log vich. 1 file (test only, app UI nahi) |
| `0008-phase7-negpadding-fix.patch` | `f0bfd5e` | blank-screen test ne asli bug pakda: `_NavPunch` vich `EdgeInsets.only(top: -18)` = negative padding = `padding.isNonNegative` assertion (debug/test crash; release vich assert strip hove janda a). Raise eh negative-Offset `Positioned` vich uthea (unclipped Stack, 42px layout slot — same look, same layout). 1 file |
