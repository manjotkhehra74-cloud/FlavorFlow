...
echo "=== FF-TOTPFIX4 $(date) ==="

node - <<'JS'
const fs = require('fs'), cp = require('child_process');
const f = '/opt/flavorflow/server/routes/auth.js';
let src = fs.readFileSync(f, 'utf8');
if (src.includes("'/totp/status', authRequired")) { console.log('ALREADY PATCHED — skip'); process.exit(0); }
...
JS
RC=$?
if [ $RC -ne 0 ]; then echo "TOTPFIX4 FAIL"; exit 1; fi
...
echo ""
echo "TOTPFIX4 VERIFIED ✓ — app ch logout/login karke Settings kholo: 2FA switch dikhega"
