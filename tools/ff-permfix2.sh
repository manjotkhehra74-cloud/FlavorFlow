...
echo "=== FF-PERMFIX2 $(date) ==="

TS=$(date +%s)
mkdir -p /opt/flavorflow/backups
cp -a data/erp.db "/opt/flavorflow/backups/erp.db.bak-perm2-$TS" 2>/dev/null
cp -a rbac.js "/opt/flavorflow/backups/rbac.js.bak-perm2-$TS"
cp -a routes/packing.js "/opt/flavorflow/backups/packing.js.bak-perm2-$TS"
echo "BACKUPS: erp.db / rbac.js / packing.js -> /opt/flavorflow/backups (suffix -perm2-$TS)"
...
if (!src.includes("'raw.view'")) {
  // Segment the file so each replacement only touches its own block:
  //   head = up to ROLE_PERMISSIONS (contains ALL_PERMISSIONS)
  //   mid  = ROLE_PERMISSIONS block only
  //   tail = NAV_ITEMS onwards (must NOT be touched by the role regex!)
  const roleIdx = src.indexOf("const ROLE_PERMISSIONS");
  const navIdx = src.indexOf("const NAV_ITEMS");
  if (roleIdx === -1 || navIdx === -1 || navIdx < roleIdx) { console.log('rbac markers NOT FOUND'); process.exit(2); }
  let head = src.slice(0, roleIdx);
  let mid = src.slice(roleIdx, navIdx);
  const tail = src.slice(navIdx);
  head = head.replace("'packing.view', 'packing.manage',",
    "'packing.view', 'packing.manage',\n  'raw.view', 'raw.manage',\n  'loss.view', 'loss.manage',");

  // ROLE_PERMISSIONS block only: mirror packing perms for every role.
  mid = mid.replace(/'packing\.view',(\s*'packing\.manage',)?/g, (m, mg) =>
    mg ? "'packing.view', 'packing.manage', 'raw.view', 'raw.manage', 'loss.view', 'loss.manage',"
       : "'packing.view', 'raw.view', 'loss.view',");

  src = head + mid + tail;
  changed = true;
  console.log('rbac: permissions added (ALL + role mirrors, NAV untouched)');
...
JS
[ $? -ne 0 ] && { echo "RBAC PATCH FAILED — restoring"; cp -a "/opt/flavorflow/backups/rbac.js.bak-perm2-$TS" rbac.js; exit 1; }
...
JS
[ $? -ne 0 ] && { echo "PACKING PATCH FAILED — restoring"; cp -a "/opt/flavorflow/backups/packing.js.bak-perm2-$TS" routes/packing.js; exit 1; }
...
if [ $? -ne 0 ]; then
  ...
  cp -a "/opt/flavorflow/backups/rbac.js.bak-perm2-$TS" rbac.js
  cp -a "/opt/flavorflow/backups/packing.js.bak-perm2-$TS" routes/packing.js
  exit 1
fi
systemctl restart flavorflow
sleep 2
curl -s -o /dev/null -w 'health -> %{http_code}\n' -m 8 http://127.0.0.1:4000/api/health || true
grep -c "raw.view" rbac.js >/dev/null && echo "PERMFIX2 VERIFIED ✓ — raw.view/raw.manage/loss.view/loss.manage live; Edit User vich nave chips dikhenge"
