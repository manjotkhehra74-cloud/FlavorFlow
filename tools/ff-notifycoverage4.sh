#!/usr/bin/env bash
# FlavorFlow notification coverage fix 4
...
echo "=== FF-NOTIFYCOVERAGE4 $(date) ==="

TS=$(date +%s)
mkdir -p "$BACKUP_ROOT"
SERVER_BAK="$BACKUP_ROOT/server.js.bak-notifycoverage4-$TS"
HELPER_BAK="$BACKUP_ROOT/helpers.js.bak-notifycoverage4-$TS"
...
if [ -f notify_all.js ]; then
  NOTIFY_EXISTED=1
  NOTIFY_BAK="$BACKUP_ROOT/notify_all.js.bak-notifycoverage4-$TS"
  cp -a notify_all.js "$NOTIFY_BAK"
  echo "BACKUP: notify_all.js -> $NOTIFY_BAK"
fi

if [ -f data/erp.db ]; then
  DB_BAK="$BACKUP_ROOT/erp.db.bak-notifycoverage4-$TS"
...
'use strict';
/* ff-notifycoverage4: permission-independent notification coverage. */
const db = require('./db');
let columns;
...
function patchServer(source) {
  if (source.includes('ff-notifycoverage4-middleware')) return source;
...
if (!testedServer.includes('ff-notifycoverage4-middleware') || testedServer.indexOf('notify_all') > testedServer.indexOf("app.use('/api/products'")) {
...
if (!testedHelper.includes('ff-notifycoverage4-audit-disabled') || testedHelper.includes('ffBroadcast(...args)')) {
...
try {
  // This server's db module is a small wrapper and does not expose
  // better-sqlite3's transaction() helper. SQL SAVEPOINT works through the
  // wrapper's stable exec() API and guarantees that probe rows never remain.
  db.exec('SAVEPOINT ff_notify_probe');
  savepointOpen = true;
  ...
  db.exec('ROLLBACK TO ff_notify_probe');
  db.exec('RELEASE ff_notify_probe');
  savepointOpen = false;
} catch (e) {
  if (savepointOpen) {
    try { db.exec('ROLLBACK TO ff_notify_probe'); } catch (_) {}
    try { db.exec('RELEASE ff_notify_probe'); } catch (_) {}
  }
  throw e;
}
const retained = Number(db.prepare("SELECT COUNT(*) n FROM notifications WHERE body LIKE '%__FF_NOTIFY_PROBE__%'").get().n);
if (retained !== 0) throw new Error(`PROBE cleanup failed: ${retained} test rows retained`);
console.log(`DB SAVEPOINT PROBE OK: ${expected}/${expected} active users targeted; 0 test rows retained`);
JS
fi
...
trap - ERR
echo "NOTIFYCOVERAGE4 VERIFIED ✓ — har successful entry SARE active users nu, permission hove jaan na hove"
